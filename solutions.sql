...
WITH
	...
	t2 AS (	
		SELECT t1.title_id, t1.au_id, sum(t1.sales_royalties) AS total_sales
		FROM t1
		GROUP BY t1.title_id, t1.au_id)

SELECT t2.au_id, sum(t2.total_sales) AS total_sales
FROM t2
GROUP BY t2.au_id
...
FROM (	
	...
	GROUP BY t1.title_id, t1.au_id) AS t2
GROUP BY t2.au_id
ORDER BY total_sales DESC
lIMIT 3;
...
SELECT temp.au_id, temp.total_sales AS profits
FROM (
		WITH
		t1 AS (
			SELECT tt.title_id,
	...
	FROM t2
	GROUP BY t2.au_id
	ORDER BY total_sales DESC) AS temp;
