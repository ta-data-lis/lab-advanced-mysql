SELECT 
    t5.au_id, ROUND((t5.total_royalties + t6.advance)) AS profit
FROM
    (SELECT 
        t4.title_id,
            t4.au_id,
            SUM(t4.sales_royalty) AS total_royalties
    FROM
        (SELECT 
        t1.title_id,
            t2.au_id,
            (t1.price * t3.qty * t1.royalty / 100 * t2.royaltyper / 100) AS sales_royalty
    FROM
        titles AS t1
    ...
    GROUP BY t4.au_id , t4.title_id) t5
        INNER JOIN
    titles t6 ON t5.title_id = t6.title_id
...
SELECT 
    temp_2.au_id,
    ROUND((temp_2.total_royalties + titles.advance)) AS profit
FROM
    temp_2
        INNER JOIN
    titles ON temp_2.title_id = titles.title_id
LIMIT 3;

-- 

CREATE TABLE publications.most_profiting_authors AS SELECT temp_2.au_id,
    ROUND((temp_2.total_royalties + titles.advance)) AS profit FROM
    temp_2
        INNER JOIN
    titles ON temp_2.title_id = titles.title_id;
